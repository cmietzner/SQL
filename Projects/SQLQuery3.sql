...
select * from EmployeeTimeOff where ID in (select TimeOffRequestID from TORDistribution where ID in 
(select EmployeeEarning.TORDistributionID from EmployeeEarning join
(select SkillID, EarningDate from EmployeeEarning where TORDistributionID is not null 
group by SkillID, EarningDate having COUNT(*) > 1) as test
on EmployeeEarning.SkillID = test.SkillID and EmployeeEarning.EarningDate = test.EarningDate
where EmployeeEarning.TORDistributionID is not null
)) and Status not in ('A')